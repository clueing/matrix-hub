...
4. **账号凭证导出与导入机制**：
   - **单账号 / 批量导出**：支持将账号的基础元数据（平台、昵称、UID、分组）与其 Playwright `storage_state.json`（全量 Cookies 与 LocalStorage）打包导出为标准 `.json` 或 `.zip` 归档文件，方便多设备迁移、容灾备份与分工交接。
   - **安全导入与自动校验**：支持上传凭证归档包，系统自动校验格式合法性、还原会话环境到 `data/sessions/{account_id}/`，并在导入完成后自动触发一次后台轻量级健康检测，直接确认登录可用性。
