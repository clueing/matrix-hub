...
- [x] 创建项目目录结构与 Python 虚拟环境配置。
- [x] 编写 `requirements.txt`（精简依赖：`fastapi`, `uvicorn`, `playwright`, `playwright-stealth`, `sqlalchemy`, `apscheduler`, `pydantic`, `httpx` 等，无需 FFmpeg 依赖）。
- [x] 搭建 FastAPI 基础架构，配置 SQLite 数据库 ORM 模型与数据表自动建表。
- [x] 实现 WebSocket 日志广播与状态推送机制。

### 阶段二：浏览器驱动层与账号授权系统 (Milestone 2)
- [x] 实现 `BrowserDriverProvider`，支持独立 `user_data_dir` 隔离。
- [x] 实现 `BasePublisherAdapter` 抽象基类标准接口。
- [x] 实现**小红书适配器**：
...
- [x] 实现**抖音适配器**：
  ...
  - 会话状态持久化与有效性探测。
- [x] 账号管理 API：支持账号列表、分组标签、状态刷新与一键登录。
- [x] 账号凭证导入与导出服务：支持单账号及批量导出为 ZIP 归档包；支持解析导入会话凭证并自动触发健康状态检测。

### 阶段三：原始视频素材服务与分发编排 (Milestone 3)
- [x] 实现原视频管理服务：本地视频路径有效性校验、基本格式校验（mp4/mov 等）、生成临时预览海报图。
- [x] 矩阵发布编排器：
  - 1:N 映射（单视频广播到多账号）。
...
- [x] 完成**抖音自动化发布流程**：
  ...
  - 平台原生定时发布控件操作与最终提交。
- [x] 任务调度器（APScheduler 集成）：
  - 多账号错峰排队算法（支持配置基础间隔与随机扰动时间）。
  - 任务失败自动重试机制与状态流转。
  - 告警服务：对接飞书/钉钉/企业微信机器人 Webhook 推送。

### 阶段五：前端可视化管理后台开发 (Milestone 5)
- [x] 初始化 Vue 3 + Vite + Element Plus 前端工程。
- [x] **账号管理界面**：账号卡片列表、分组筛选、扫码登录弹窗（实时展示二维码）、一键健康检查、唤起人工辅助、账号凭证一键导出与批量导入。
- [x] **矩阵分发工作台**：
  - 视频与账号映射配置（支持“1视频->多账号”、“多视频->对应账号”）。
  - 统一母版配置 + 账号级独立覆盖编辑（标题、标签、定时时间）。
  - 文件夹批量扫描与 Excel 任务导入导出支持。
- [x] **任务看板与调度中心**：任务时间轴、实时日志流展示、手动重试/取消。
- [x] **系统设置**：错峰时间参数、Webhook 机器人 URL、平台扩展配置。

### 阶段六：Windows 一键启动与多平台扩展验证 (Milestone 6)
- [x] 编写前端构建与静态文件集成脚本（FastAPI 直接托管编译后的前端资源）。
- [x] 编写 Windows 一键启动脚本 `start.bat`（自动检查依赖、启动后台、自动在默认浏览器打开控制台）。
- [x] 验证小红书和抖音端到端发布全链路。
- [x] 为快手与微信视频号提供规范化适配器接口骨架与开发指南。
